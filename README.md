...
The `comparison` result keeps the decision rules separate: `preferred` follows
the legacy chi-squared deviance test, while `preferred_by_AIC` and
...
`consensus_model_decision()` treats the chi-squared comparison, AIC, and BIC as
three model-selection votes:

```r
consensus_model_decision(analysis$comparison)
```

The result reports each vote, the total per model, and an agreement level of
`unanimous`, `majority`, or `no_consensus`. This is a relative selection summary
only; use `analysis$assessment` to determine whether the selected model also
