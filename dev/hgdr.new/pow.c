 ...
 * mth_pow overflow     x**y > MTH_MAXNUM      INFINITY
 * mth_pow underflow   x**y < 1/MTH_MAXNUM       0.0
 * mth_pow domain      x<0 and y noninteger  0.0
 *
 */
...
#define douba(k) arr_a[k]
#define doubb(k) arr_b[k]
#define MEXP 16383.0
#ifdef WANT_DENORMAL
...
*/
#define douba(k) (*(double *)&arr_a[(k)<<2])
#define doubb(k) (*(double *)&arr_b[(k)<<2])
#define MEXP 2031.0
#define MNEXP -2031.0
#endif
...
/* log2(e) - 1 */
#define MTH_LOG2EA 0.44269504088896340736

	/* AvK: dit snap ik niet.
	** Stamt waarschijnlijk uit een opgraving van een bosmongolendorp
	*/
#define F W
#define Fa Wa
#define Fb Wb
...
#if 0
w = y * WRAP_log(x);
if (aw > 1.0e-3 && WRAP_fabs(w) < 1.0e-3)
  {
    z = ((((((
...
/* Find significand of x in antilog table arr_a[]. */
i = 1;
if ( x <= douba(9) )
	i = 9;
if ( x <= douba(i+4) )
...
/* Find (x - arr_a[i])/arr_a[i]
 * in order to compute log(x/arr_a[i]):
 *
 ...
 */
z = x*x;
w = x * ( z * mth_polevl( x, arr_p, 3 ) / mth_p1evl( x, arr_q, 4 ) );
w = w - WRAP_ldexp( z, -1 );   /*  w - 0.5 * z  */
...
H = Fb + Gb;
Ha = reduc(H);
w = WRAP_ldexp( Ga+Ha, 4 );

/* Test the power of 2 for overflow */
if ( w > MEXP )
	{
#ifndef WANT_INFINITIES
	mtherr( fname, MTHE_OVERFLOW );
#endif
	if ( nflg && yoddint )
	  return -MTH_INFINITY ;
	return MTH_INFINITY ;
...
if ( w < (MNEXP - 1) )
	{
#ifndef WANT_DENORMAL
	mtherr( fname, MTHE_UNDERFLOW );
#endif
	if ( nflg && yoddint )
	  return MTH_NEGZERO ;
	return 0.0 ;
 ...
 * where -0.0625 <= Hb <= 0.
 */
z = Hb * mth_polevl( Hb, arr_r, 6 );  /*    z  =  2**Hb - 1    */
...
z = WRAP_ldexp( z, i );  /* multiply by integer power of 2 */
