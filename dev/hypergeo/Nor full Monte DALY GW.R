...
#Import Pathogen Concentrations
setwd("C:")
#Ca CFU/L; Cr oocysts/L; No gc/L; Ro FFU/L

ConcNoGW1<- read.csv("ConcNoComb1000.csv",
               header = TRUE,
               quote="\"",
...
for(ii in 1:reps){
	N<-50
	pathden<-sample(log(ConcNoGW1[,ii+1],10),N)
	noro<-function(X){
		risk<-ill<-rep(0,reps)
			for (i in 1:reps){
...
print(quantile(arisk5,c(.05,.5,.95,.99)))}
...
for(ii in 1:reps){
	...
	pathden<-sample(log(ConcNoGW1[,ii+1],10),N)
	N<-1
	pathden1<-sample(log(ConcNoGW1[,ii+1],10),N)
	noro<-function(X){
		risk<-ill<-rep(0,reps)
			for (i in 1:reps){
				risk[i]=Probinfbpa(a=((exp(ot[i,1]+ot[i,2])/(1+exp(ot[i,1])))),b=exp(ot[i,2])* (1-(exp(ot[i,1])/(1+exp(ot[i,1])))),X)
...
arisk6[ii]<-(.9*(arisk4[ii])+.1*(1-(1-arisk4[ii])*(1-arisk5[ii])))#*Daly.case[i]
	...
	arisk8[ii]<-(1-prod(1-tase2[1,]))#*Daly.case[i]

arisk9[ii]<-(.9*(arisk7[ii])+.1*(1-(1-arisk7[ii])*(1-arisk8[ii])))*Daly.case[ii]
}
print(quantile(arisk9,c(.05,.5,.95,.99)))}
