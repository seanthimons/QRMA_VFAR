...
##########NEW you can break at the end of teach t iteration
for(t in 1:3){
#Irrigation
tase<-0
...
return(f1_results)
  ...
  loop1 <- lapply(ftr1, FUN = value)
  
  saveRDS(loop1, file = paste0('~/marystemp/loop1_daly_sw',as.numeric(Sys.time()),'.RDS'))
  toc()
  rm(ftr1)
}
...
V<-2
LRT<-test[t]
##########NEW you can break at the end of teach ii iteration
for(ii in 1:reps){
	N<-365
	pathden<-rnorm(N,4.7,1.5)-1
...
arisk6[ii]<-(.9*(arisk4[ii])+.1*(1-(1-arisk4[ii])*(1-arisk5[ii])))#*Daly.case[i]
	...
	arisk8[ii]<-(1-prod(1-tase2[1,]))#*Daly.case[i] #########NEW I changed this to tase2 to capture the day of high volume consumption
...
return(f2_results)
  ...
  loop2 <-lapply(ftr2, FUN = value)
  
  saveRDS(loop2, file = paste0('~/marystemp/loop2_daly_sw',as.numeric(Sys.time()),'.RDS'))
  toc()
  rm(ftr2)
}
