...
{
loop1_under <- map_dfr(loop1_results, 1) %>% unlist()
loop1_target <- map_dfr(loop1_results, 2) %>% unlist()
loop1_over <- map_dfr(loop1_results, 3) %>% unlist()

  {
quantile(loop1_under, c(0.05, .5, .95, .99))
quantile(loop1_target, c(0.05, .5, .95, .99))
quantile(loop1_over, c(0.05, .5, .95, .99))
  }



loop2_1 <- readRDS("~/marystemp/loop2_1664353432.61491.RDS")
loop2_2 <- readRDS("~/marystemp/loop2_1664309056.10483.RDS")
...
setwd('~/marystemp/')
fileConn <- file('loop2_results.txt')
{cat('5%              50%             95%         99%','\n', file = "loop2_results.txt", append = TRUE)
